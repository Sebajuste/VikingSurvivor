...
func update_Settings(height, width, fullscreen, vsync, antialiasing, audio, mute):
	...
	Settings.Audio.EFFECTS = audio.z
...
func apply_settings():
	...
	DisplayServer.window_set_vsync_mode(DisplayServer.VSYNC_ADAPTIVE if Settings.Display.Vsync else DisplayServer.VSYNC_DISABLED )
	
	#get_viewport().msaa = Viewport.MSAA_4X
	
	print("AudioServer.get_bus_index(Master): ", Settings.Audio.MASTER)
	...
	AudioServer.set_bus_volume_db(AudioServer.get_bus_index("Effects"), Settings.Audio.EFFECTS)
	
	
	AudioServer.set_bus_mute(AudioServer.get_bus_index("Master"), Settings.Audio.MUTE)
	AudioServer.set_bus_mute(AudioServer.get_bus_index("Music"), Settings.Audio.MUTE)
	AudioServer.set_bus_mute(AudioServer.get_bus_index("Effects"), Settings.Audio.MUTE)
